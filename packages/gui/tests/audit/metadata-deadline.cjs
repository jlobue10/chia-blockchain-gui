...
function environment() {
  let now = 0, id = 0;
  const timers = new Map(), requests = [], handlers = new Map(), exposed = {};
  const clock = { advance(ms) {
      const target = now + ms;
      for (;;) {
        let next;
        for (const [key, t] of timers)
          if (t.at <= target && (!next || t.at < next[1].at))
            next = [key, t];
        if (!next)
          break;
        now = next[1].at;
  ...
  class Request extends EventEmitter {
    constructor(url) { super(); this.url = url; this.aborted = false; requests.push(this); }
    setHeader() { }
    end() { this.ended = true; }
    abort() { if (!this.aborted) {
      this.aborted = true;
      this.emit('abort');
    } }
    response(status = 200) { const r = new EventEmitter(); r.statusCode = status; r.headers = { 'content-type': 'application/json' }; this.emit('response', r); this.incoming = r; return r; }
    succeed(text) { const r = this.response(); r.emit('data', Buffer.from(text)); r.emit('end'); }
  }
  const cache = new Map();
  const moduleDefault = x => ({ __esModule: true, default: x });
  const state = { CACHED: 'CACHED', ERROR: 'ERROR', NOT_CACHED: 'NOT_CACHED' };
  const electron = { net: { request: url => new Request(url) }, BrowserWindow: class {
    }, dialog: { showOpenDialog: async () => ({ canceled: true }) },
    contextBridge: { exposeInMainWorld: (name, value) => exposed[name] = value },
    ipcRenderer: { invoke: (channel, ...args) => handlers.get(channel)(...args), on() { }, off() { } } };
  const sandbox = { console, Buffer, TextDecoder, AbortController, Error, Date: TestDate,
    setTimeout: (fn, ms) => { timers.set(++id, { at: now + Number(ms), fn }); return id; }, clearTimeout: key => timers.delete(key) };
  ...
  function load(file) {
    ...
    const result = ts.transpileModule(source, { fileName: file, compilerOptions: { target: ts.ScriptTarget.ES2022, module: ts.ModuleKind.CommonJS, esModuleInterop: true }, reportDiagnostics: true });
    assert.equal(result.diagnostics?.filter(d => d.category === ts.DiagnosticCategory.Error).length, 0, file + ' parse diagnostics');
    ...
    function localRequire(name) {
      ...
      if (name === './useCache')
        return moduleDefault(() => cacheBinding);
      if (name.endsWith('/compareChecksums'))
        return moduleDefault((a, b) => a.replace(/^0x/, '') === b.replace(/^0x/, ''));
      if (name.endsWith('/parseFileContent'))
        return moduleDefault(content => new TextDecoder().decode(content));
      if (name.endsWith('/CacheState'))
        return moduleDefault(state);
      if (name.endsWith('/isValidURL'))
        return moduleDefault(s => typeof s === 'string' && /^https:\/\//.test(s));
      if (name.endsWith('/ipfsGateway'))
        return { IpfsGatewayDisabledError: class extends Error {
          }, toFetchableUrl: s => s };
      if (name.endsWith('/ipcMainHandle'))
        return moduleDefault((name, fn) => handlers.set(name, fn));
      if (name.endsWith('/ensureDirectoryExists'))
        return moduleDefault(p => fsp.mkdir(p, { recursive: true }));
      if (name.endsWith('/getChecksum'))
        return moduleDefault(async (p) => crypto.createHash('sha256').update(await fsp.readFile(p)).digest('hex'));
      if (name.endsWith('/fileExists'))
        return moduleDefault(async (p) => { try {
          await fsp.stat(p);
          return true;
        }
        catch {
      ...
      if (name.endsWith('/sanitizeFilename'))
        return moduleDefault(s => s);
      if (name.endsWith('/sanitizeNumber'))
        return moduleDefault(s => Number(s));
      if (name.endsWith('/limit'))
        return load(path.join(root, 'util/limit.ts'));
      if (name === './constants/API' || name === './API')
        return moduleDefault({ APP: 'app', PREFERENCES: 'preferences', CHIA_LOGS: 'chiaLogs', LINK: 'link', PERMISSIONS: 'permissions', ADDRESS_BOOK: 'addressBook', CACHE: 'cache', WEBSOCKET: 'websocket' });
...
test('invalid IPC durations are refused before any request', async () => withManager(async ({ manager, requests }) => {
...
test('active trickle hits absolute deadline, aborts real downloader, deletes temp and releases slot', async () => withManager(async ({ manager, requests, clock, dir }) => {
  ...
  requests[1].succeed('{}');
  await second;
  assert.equal((await fsp.readdir(dir)).some(f => f.endsWith('.tmp')), false);
...
test('a first metadata request queued longer than its allowance still gets its full transfer share', async () => withManager(async ({ manager, requests, clock }) => {
...
test('metadata joining an active video-sized request tightens from the original start', async () => withManager(async ({ manager, requests, clock }) => {
...
test('joining after the shorter allowance is already consumed aborts immediately', async () => withManager(async ({ manager, requests, clock }) => {
...
test('longer coalesced callers cannot extend a short deadline', async () => withManager(async ({ manager, requests, clock }) => {
...
test('queued coalesced callers retain their tighter budget without spending it in the queue', async () => withManager(async ({ manager, requests, clock }) => {
...
test('content+headers+checksum returns a single network result and hashes returned bytes', async () => withManager(async ({ manager, requests }) => {
...
test('bundle reads honor the migration barrier from PR3068', async () => withManager(async ({ manager, requests }) => {
  ...
  await first;
  let release;
  manager.maintenance = new Promise(resolve => { release = resolve; });
  let completed = false;
  const pending = manager.getContentWithInfo(url, { maxDuration: 1000 }).then(value => { completed = true; return value; });
  for (let i = 0; i < 50; i++)
    await new Promise(setImmediate);
...
test('metadata helper reserves <=60 seconds across all five transfers including the first', async () => {
  ...
  const calls = [];
  await assert.rejects(helper.default(Array.from({ length: 20 }, (_, i) => 'https://test.invalid/' + i), 'hash', async (u, h, o) => { calls.push(o.maxDuration); env.clock.advance(o.maxDuration); throw new Error('failed'); }));
  assert.equal(calls.length, 5);
  assert.equal(calls.reduce((a, b) => a + b, 0), 60000);
...
test('metadata helper preserves no-hash single URI, first success and mismatch priority', async () => {
  ...
  let count = 0;
  await assert.rejects(fetch(['a', 'b'], undefined, async (u, h, o) => { count++; assert.equal(u, 'a'); assert.equal(o.maxDuration, 60000); throw new Error('failed'); }));
  assert.equal(count, 1);
  count = 0;
  assert.equal(await fetch(['a', 'b'], 'hash', async () => { count++; return 42; }), 42);
  assert.equal(count, 1);
  await assert.rejects(fetch(['a', 'b'], 'hash', async (u) => { throw new Error(u === 'b' ? CHECKSUM_MISMATCH_ERROR : 'network'); }), /Checksum mismatch/);
});
test('metadata wrapper passes one bounded call and validates before parsing', async () => {
  const env = environment();
  let calls = 0;
  env.bind({ getContentWithInfo: async (u, o) => { calls++; assert.equal(o.maxDuration, 12000); return { checksum: 'hash', headers: {}, content: Buffer.from('{"ok":true}') }; } });
...
test('preload forwards the bounded options to the registered main-process bundle handler', async () => withManager(async (env) => {
...
test('write-open failure rejects rather than leaking a slot', async () => withManager(async ({ manager, requests, dir }) => {
